...
const resources = {
  en: { translation: {
    ...
    networkCpuLive: 'Network CPU Usage (Live)',
    ...
    networkTraffic: 'Network Traffic (Bytes/sec)',
  ...
  ja: { translation: {
    ...
    networkCpuLive: 'ネットワーク CPU 使用率 (ライブ)',
    ...
    analytics: '分析',
    uptimeLabel: '連続稼働',
    resourceChart: 'リソースチャート',
    loadingData: 'データ読み込み中... ⏳',
    cpuUsage: 'CPU使用率',
    memoryUsage: 'メモリ使用率',
    diskUsage: 'ディスク使用率',
    networkTraffic: 'ネットワークトラフィック (Bytes/sec)',
  ...
  ko: { translation: {
    ...
    networkCpuLive: '네트워크 CPU 사용률 (라이브)',
    ...
    analytics: '분석',
    uptimeLabel: '연속 가동',
    resourceChart: '리소스 차트',
    loadingData: '데이터 로딩 중... ⏳',
    cpuUsage: 'CPU 사용률',
    memoryUsage: '메모리 사용률',
    diskUsage: '디스크 사용률',
    networkTraffic: '네트워크 트래픽 (Bytes/sec)',
    downloadRx: '다운로드 (수신)',
    uploadTx: '업로드 (송신)',
...
export function formatTooltipTime(lang, val) {
  ...
  if (!isNaN(d.getTime())) {
    const locale = lang === 'zh' ? 'zh-CN' : lang === 'ko' ? 'ko-KR' : lang === 'ja' ? 'ja-JP' : 'en-US';
    return d.toLocaleString(locale); // ツールチップ用には日付と時刻の両方を表示
  }
  return val;
}
